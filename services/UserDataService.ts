...
export class UserDataService {
  ...
  static async testConnection(): Promise<boolean> {
    try {
      ...
      console.error(
        "UserDataService - Firestore connection failed, using local storage fallback",
      );
      console.error("UserDataService - Connection error code:", error?.code);
      console.error(
        "UserDataService - Connection error message:",
        error?.message,
      );
  ...
  static async getUserData(userId: string): Promise<UserData | null> {
    try {
      ...
      console.error(
        "UserDataService - Error fetching user data from Firestore, trying local",
      );
      console.error(
        "UserDataService - Firestore error:",
        error?.code,
        error?.message,
      );
      ...
      try {
        ...
        console.error(
          "UserDataService - Local storage also failed:",
          localError,
        );
        return null;
      }
  ...
  static async updateUserStats(
    ...
    try {
      ...
      console.error(
        "UserDataService - Error updating stats in Firestore, trying local",
      );
      console.error(
        "UserDataService - Firestore error:",
        error?.code,
      ...
      try {
        ...
        console.error(
          "UserDataService - Local storage update also failed:",
          localError,
        );
        throw error;
      }
  ...
  static async addTask(
    ...
    try {
      ...
      console.error(
        "UserDataService - Error adding task to Firestore, trying local storage",
      );
  ...
  static async retryOperation<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        console.error(
          `UserDataService - Attempt ${attempt} failed:`,
          error?.code,
          error?.message,
        );
  ...
  static async getUserTasks(userId: string): Promise<Task[]> {
    try {
      ...
      console.error(
        "UserDataService - Error fetching from Firestore, trying local storage",
      );
      console.error(
        "UserDataService - Firestore error:",
        error?.code,
  ...
  static async completeTask(taskId: string, userId?: string): Promise<void> {
    try {
      ...
      console.error(
        "UserDataService - Error completing task in Firestore:",
        error,
      );
      ...
      if (userId) {
        try {
          ...
          console.error(
            "UserDataService - Local storage completion also failed:",
            localError,
          );
        }
      }
  ...
  static async getCompletedTasks(userId: string): Promise<Task[]> {
    try {
      ...
      console.error(
        "UserDataService - Error fetching completed tasks from Firestore:",
        error,
      );
  ...
  static async resetUserData(userId: string): Promise<void> {
    try {
      ...
      const deleteTaskPromises = tasksSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref),
      );
      ...
      const deleteVoucherPromises = vouchersSnapshot.docs.map((doc) =>
        deleteDoc(doc.ref),
      );
